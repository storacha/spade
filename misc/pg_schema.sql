...
CREATE TABLE IF NOT EXISTS spd.datasets (
  ...
  CONSTRAINT tenant_dataset_ref UNIQUE ( tenant_id, dataset_id ), -- pieces_datasets FK
  CONSTRAINT tenant_dataset_slug UNIQUE ( dataset_slug, tenant_id )
);
...
CREATE INDEX IF NOT EXISTS published_deals_provider_id_idx ON spd.published_deals ( provider_id );
CREATE INDEX IF NOT EXISTS published_deals_client_id_idx ON spd.published_deals ( client_id );
